...
Node.prototype.goto = function(node){
    var name = 'path:' + this.name + "@" + node.name;
    this.origin(name);
    node.target(name);
...
Node.prototype.brokenArrow = function(){
  if (this.flowtex.lastAction !== null
      && this.flowtex.lastAction.name == "goto"
      || this.flowtex.lastAction.name == "offsetArrow"
  ){
    var origin = this.flowtex.lastAction.origin;
    var target = this.flowtex.lastAction.target;
    this.flowtex.arrows.putArrowType(origin, target, '|-');
  }
  this.flowtex.lastAction.name = 'brokenArrow';
  return this;
};

Node.prototype.offsetArrow = function(x,y){
  if (this.flowtex.lastAction !== null
      && this.flowtex.lastAction.name == "goto"
      || this.flowtex.lastAction.name == "brokenArrow"
  ){
    var origin = this.flowtex.lastAction.origin;
    var target = this.flowtex.lastAction.target;
    this.flowtex.arrows.putArrowOffset(origin, target, {
      x: x + this.flowtex.default.unit,
      y: y + this.flowtex.default.unit
    });
  }
  this.flowtex.lastAction.name = 'offsetArrow';
  return this;
...
Node.setLabel = function(self, direction, text){
    ...
    if ( lastAction == null
        || ( lastAction.name != "goto"
              && lastAction.name != "target"
              && lastAction.name != "origin"
              && lastAction.name != "brokenArrow"
              && lastAction.name != "offsetArrow"
    ) ){
        return;
    }
